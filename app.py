...
# ═══════════════════════════════════════════════
    ...
    st.caption("🔎 Select values below to simulate different customer profiles and see predicted expenditure, savings, and WTP in real-time.")
    co1,co2,co3 = st.columns(3)
    wi_income_opts = list(ORDINAL_MAPS["Q6_Monthly_Income"])
    wi_dep_opts = list(ORDINAL_MAPS["Q4_Financial_Dependents"])
    wi_impulse_opts = list(ORDINAL_MAPS["Q10_Impulse_Purchase_Frequency"])
    wi_subs_opts = list(ORDINAL_MAPS["Q12_Active_Subscriptions"])
    wi_stress_opts = list(ORDINAL_MAPS["Q18_Financial_Stress_Level"])
    wi_literacy_opts = list(ORDINAL_MAPS["Q21_Financial_Literacy"])
    wi_confidence_opts = list(ORDINAL_MAPS["Q17_Financial_Confidence"])
    wi_stability_opts = list(ORDINAL_MAPS["Q7_Income_Stability"])
    wi_age_opts = list(ORDINAL_MAPS["Q1_Age_Group"])
    with co1:
        wi_income = wi_income_opts.index(st.selectbox("Monthly Income", wi_income_opts, index=2))
        wi_deps = wi_dep_opts.index(st.selectbox("Financial Dependents", wi_dep_opts, index=1))
        wi_impulse = wi_impulse_opts.index(st.selectbox("Impulse Purchase Frequency", wi_impulse_opts, index=1))
    with co2:
        wi_subs = wi_subs_opts.index(st.selectbox("Active Subscriptions", wi_subs_opts, index=1))
        wi_stress = wi_stress_opts.index(st.selectbox("Financial Stress Level", wi_stress_opts, index=2))
        wi_literacy = wi_literacy_opts.index(st.selectbox("Financial Literacy", wi_literacy_opts, index=2))
    with co3:
        wi_confidence = wi_confidence_opts.index(st.selectbox("Financial Confidence", wi_confidence_opts, index=2))
        wi_stability = wi_stability_opts.index(st.selectbox("Income Stability", wi_stability_opts, index=2))
        wi_age = wi_age_opts.index(st.selectbox("Age Group", wi_age_opts, index=1))
